...
processor.
\item Async NetCDF option
\begin{Verbatim}
  --enable-async-netcdf
                     Enable pthread-backed async NetCDF output and ICBC
                     prefetching.
\end{Verbatim}
This option enables asynchronous output writes and prefetching of boundary
condition input. The default build leaves these features disabled and does not
require pthreads.
\item CLM option
\begin{Verbatim}
...
\item CLM 4.5 carbon--nitrogen dynamic vegetation option
\begin{Verbatim}
  --enable-clm45 --enable-cndv
\end{Verbatim}
The CNDV option enables both the CLM carbon--nitrogen model and prognostic
dynamic vegetation. It requires CLM4.5 and must not be combined with the
DYNPFT source option. In CLM4.5, the natural vegetation patch count is derived
internally from the number of plant functional types, so the
\verb=MAXPATCH_PFT= preprocessor definition used by older CLM versions must
not be added manually. This implementation includes the RefinedCN and
ModifiedDV changes described by Wang et al. (2016,
\verb=doi:10.1007/s00382-015-2596-z=): stress-deciduous phenology uses the
driest/wettest of the top three soil layers for leaf onset/senescence and a
30-day stress-deciduous leaf-dropping period, while tropical broadleaf
evergreen trees are excluded when the recursively updated, 20-year-weighted
mean drought duration exceeds 45 days. Detailed implementation and usage
notes are in \verb=Doc/CNDV_IMPLEMENTATION_ZH.md= and
\verb=Doc/CNDV_BUILD_AND_RUN_ZH.md=.
\item OASIS option
\begin{verbatim}
...
\subsection{eCLM option}

To build the eCLM coupled model, a particular modified version of the
\verb=OASIS3-MCT5.0= libraries is needed. Yoy can get it from here:

\begin{verbatim}
https://icg4geo.icg.kfa-juelich.de/ExternalReposPublic/oasis3-mct
\end{verbatim}

Adapting one if the system configuration files in \verb=util/make_dir=.

\begin{verbatim}
  git clone https://icg4geo.icg.kfa-juelich.de/ExternalReposPublic/oasis3-mct.git
  cd oasis3-mct
  export OASIS_ROOT=$PWD
  cp util/make_dir/make.{compiler}_{mpi}_hostname util/make_dir/make.{mysystem}
  [edit util/make_dir/make.mysystem and fix paths]
  export OASIS_ENV={mysystem}
  make -f TopMakefileOasis3
\end{verbatim}

If all is ok, the OASIS static libraries will be compiled in a directory
inside \verb=$OASIS_ROOT=.

Export its path as \verb=OASIS_PREFIX=.
